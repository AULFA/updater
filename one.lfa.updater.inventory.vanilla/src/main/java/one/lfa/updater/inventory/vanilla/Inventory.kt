...
class Inventory private constructor(
  ...
  private fun opInventoryRepositoryAdd(
    uri: URI,
    requiredUUID: UUID?
  ): SettableFuture<InventoryRepositoryAddResult> {
    this.logger.debug("opInventoryRepositoryAdd: {} {}", uri, requiredUUID)

    val future = SettableFuture.create<InventoryRepositoryAddResult>()
    ...
    this.executor.execute {
      ...
      future.set(this.handleAddResult(result, uri))
    }

    return future
  }

  private fun opInventoryRepositoryRemove(
    id: UUID
  ...
  private fun handleAddResult(
    result: InventoryTaskResult<InventoryRepository>,
    uri: URI
  ...
  private fun opInventoryRepositoryPut(
  ...
  override fun inventoryRepositoryAdd(
    ...
    return this.opInventoryRepositoryAdd(uri, requiredUUID)
  }
  ...
  override fun inventoryRepositories(): List<InventoryRepositoryType> =
    synchronized(this.repositoryLock) {
      this.repositories.values.toList().sortedBy { r -> r.title }
    }

  override fun inventoryRepositoryPut(
    repository: Repository
  ): ListenableFuture<InventoryRepositoryAddResult> {
    return this.opInventoryRepositoryPut(repository)
  }

  override fun inventoryRepositoryRemove(
    id: UUID
    ...
    return this.executor.submit(Callable {
      this.opInventoryRepositoryRemove(id)
    })
  }
